...
selectBtn.onclick = () => {
  ...
  tip.textContent = '请在页面中鼠标移入列表型 <a> 标签项';
  // 注入内容脚本
  chrome.tabs.query({active:true,currentWindow:true}, tabs => {
...
function contentSelectLinks(tabId) {
  ...
  function _getSimilarElements(element, selectorCache, selectorMap) {
    ...
    if (selectorMap[fullSelector]) {
        ...
        try {
            // 获取完整选择器匹配的所有元素数量
            const totalMatches = document.querySelectorAll(fullSelector).length;
            ...
            do {
                ...
                // 如果缓存中没有，则查询DOM并缓存结果
                if (!matchedElements) {
                    matchedElements = document.querySelectorAll(bestSelector);
  ...
  */
  function _onClickElement(event, elementGroups, onSelectGroup) {
    ...
    if (elementGroups.length) {
        // 移除所有现有的高亮
        document.querySelectorAll(".feedly-selected").forEach(element => {
